...
export class YearService {
  ...
  async create(data: AddYearInput, userId: string): Promise<YearDocument> {
    ...
    return this.yearModel.create({ ...data, closed: false, userId: new Types.ObjectId(userId) });
  }

  // Get all Years
  async getYears(userId: string): Promise<YearDocument[]> {
    return this.yearModel.find({ userId: new Types.ObjectId(userId) }).exec();
  }

  // Get a Year by its year
  async getYearByYear(year: string, userId: string): Promise<YearDocument | null> {
    return this.yearModel.findOne({ year, userId: new Types.ObjectId(userId) }).exec();
  }
  async getOpenedYear(userId: string) {
    return this.yearModel.findOne({ closed: false, userId: new Types.ObjectId(userId) });
  }

  // Get a Year by its ID
  async getYearById(id: string, userId: string): Promise<YearDocument | null> {
    return this.yearModel.findOne({ _id: new Types.ObjectId(id), userId: new Types.ObjectId(userId) }).exec();
  }

  // Get all years by topic
  async getYearsByTopic(topicId: string, userId: string): Promise<YearDocument[]> {
    const yearTopics = await this.yearTopicModel.find({ topicId: new Types.ObjectId(topicId), userId: new Types.ObjectId(userId) }).exec();
    return this.yearModel.find({ _id: { $in: yearTopics.map(yt => new Types.ObjectId(yt.yearId)) }, userId: new Types.ObjectId(userId) }).exec();
  }

  // Close a year
  async closeYear(id: string, userId: string): Promise<YearDocument> {
    const year = await this.yearModel.findOne({ _id: new Types.ObjectId(id), userId: new Types.ObjectId(userId) }).exec();
    if (!year) {
